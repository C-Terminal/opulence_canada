<script lang="ts">
    ...
    */
	 ...
	 * @type {string | null}
	 */
	let focusElementStyle: string | null = null;
	let isInputFocused = false;
	let refs: { [key: string]: HTMLElement } = {};
	/**
	 * @type {string | any[]}
	 */
	let cardNumberMask: string | any[];
	...
	$: {
		...
		for (let index = 0; index < cardNumber.length; index++) {
			if (cardNumberMask[index] == ' ' && cardNumber[index] !== ' ')
				cardNumber =
					cardNumber.substr(0, index) + ' ' + cardNumber.substr(index, cardNumber.length - index);
		}
	...
	function focusInput(e: Event) {
		...
		let targetRef = (e.target as HTMLElement).dataset.ref;
		let target = refs[targetRef !== undefined ? targetRef : 'defaultRef'];
				...
				<div class="card-item__side front">
					...
					<div class="card-item__cover">
						<img
							...
							src={"/images/credit_card/" + currentCardBackground + ".jpeg"}
					...
					<div class="card-item__wrapper">
						<div class="card-item__top">
							<img
								alt="card"
								src="/images/credit_card/chip.png"
								class="card-item__chip"
							/>
                            <div class="card-item__type">
                                {#each [cardType] as type (type)}
                                  <img
                                    in:fly={{ y: -20 }}
                                    out:fly={{ y: 20 }}
							...
							{#if cardType}
								{#each [cardType] as type (type)}
									<img
										...
										out:fly={{ y: 20 }}
										alt="card"
										src={"/images/credit_card/" + cardType + ".png"}
										class="card-item__typeImg"
									/>
								{/each}
							{/if}
